...
def ssim(clean, noisy, normalized=True, raw=False):
    ...
    noisy = noisy.squeeze()
    return np.array([structural_similarity(c, n, data_range=n.max() - n.min(), multichannel=True) for c, n in zip(clean, noisy, strict=False)]).mean()
...
def psnr(clean, noisy, normalized=True, raw=False):
    ...
    return np.array([peak_signal_noise_ratio(c, n, data_range = 1.0) for c, n in zip(clean, noisy, strict=False)]).mean()
