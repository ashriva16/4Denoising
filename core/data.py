...
"""
This module contains utilities to load training data and the live diffraction
pattern for denoising into pytorch.

Functions will be added in due course.

All assumes that data is held in memory as numpy files currently.

If necessary, we could investigate modifying this to holding data as cupy arrays
if there is a suitable GPU with enough memory to hold these.
"""
...
class STEMDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, file_path, samplershape="5l"):
        """
        A deliberate choice is made to just select one file to map from, so
        self.imgs only has one item. I would recommend upgrading this
        software so it can handle multiple "images" (actually 4D datasets)
    ...
    def __len__(self):
        ...
        """
        return (self.imgs[0].shape[0] - self.top_exclude - self.bottom_exclude) * (
            self.imgs[0].shape[1] - self.left_exclude - self.right_exclude
        )

    # Note: uses (x,y) while the rest uses numpy (y,x), so shapes can look odd;
    ...
    def selector(self, Rx_pos, Ry_pos):
        ...
        -------
        DPs: np.ndarray
            A 3D array of dimensions (n,Qx,Qy); n is the number of diffraction
            patterns returned for video denoising.

        """
        samplershape = self.samplershape
        assert samplershape in [
            "5l",
            "3d",
            "3s",
        ], "Undefined Shape Code, please choose 5l, 3d, 3s"
        # Because of the sampler, we have to downsample; we cannot train with all
        # pixels of the image, so we exclude some at the edges.
        ...
        keepBR = np.where(
            np.logical_and(shifted_slicer_1[0] < self.Rx, shifted_slicer_1[1] < self.Ry)
        )[0]
    ...
    def getitem(self, index):
        ...
        if item_input.shape[0] != self.n:
            raise ValueError(
                "Incorrect number of channels. This is probably because, somehow,"
